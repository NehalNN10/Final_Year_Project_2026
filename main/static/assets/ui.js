...
export function setupGUI(frameUpdateCallback) {
    ...
    //     const dx = camera.position.x - intersectionPoint.x;
    //     const dz = camera.position.z - intersectionPoint.z;
    //     const radius = Math.sqrt(dx * dx + dz * dz);
    //     camera.position.x = intersectionPoint.x + radius * Math.sin(angle);
    //     camera.position.z = intersectionPoint.z + radius * Math.cos(angle);
    //     controls.update();
    // });

    // const posX = gui_cam.add(params, 'x', -50, 50, 0.1).name("Position X").listen().onChange((val) => {
    //     const delta = val - intersectionPoint.x;
    //     controls.target.x = val;
    //     camera.position.x += delta;
    // });

    // const posZ = gui_cam.add(params, 'z', -50, 50, 0.1).name("Position Z").listen().onChange((val) => {
    //     const delta = val - intersectionPoint.z;
